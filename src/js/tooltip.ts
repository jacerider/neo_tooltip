  ...
  Drupal.behaviors.neoTooltip = {
    ...
    getOptions: (el: HTMLElement) => {
      ...
      if (triggerToNearest) {
        ...
        // in the page's top-left corner. ElementProcess draws no badge in that
        // case, and this is the guard for any that reach here another way.
        const onScreen = (n:HTMLElement|null) =>
          n && !n.closest('.visually-hidden, .sr-only') ? n : null;
        // The badge, found by the id both ends carry.
        //
        // Never by position. The badge is not always inside the form item —
        // a control with no visible label of its own has its badge drawn on
        // the enclosing legend — and the form item is not always this
        // element's own: something with no form item at all, a submit button
        // among them, sits inside the nearest ancestor that is one, so a
        // search upward from the button lands in the enclosing group and
        // claims that group's badge. Every Remove button in an Alchemist card
        // list ended up anchored to the one question mark in the legend, and
        ...
        const anchor =
          // The help badge for this element, wherever it was drawn.
          onScreen(badge)
